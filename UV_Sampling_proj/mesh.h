...
struct R_Mesh
{
	...
	std::vector<cv::Vec3f> calcVertNorm(std::vector<cv::Vec3f>& newVerts)
	{
		std::vector<cv::Vec3f> normArray(numV(), cv::Vec3f(0., 0., 0.));
		std::vector<double> sumWei(numV(), 0.);
		for (int f = 0; f < numF(); f++)
		{
			cv::Vec3i find = faceInds[f];
			cv::Vec3f d0 = newVerts[find[1]] - newVerts[find[0]];
			cv::Vec3f d1 = newVerts[find[2]] - newVerts[find[1]];
			cv::Vec3f fN = normalize(d0.cross(d1));
			float area = norm(fN) * 0.5;
			for (int d = 0; d < 3; d++)
			{
	...
	std::vector<std::vector<int>> calcVertAdjFaceID()
	{
		std::vector<std::vector<int>> adjFace(numV(), std::vector<int>());
		for (int f = 0; f < numF(); f++)
		{
			cv::Vec3i FF = faceInds[f];
			for (int d = 0; d < 3; d++)
