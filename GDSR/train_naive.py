...
def one_iteration_network_training(caseInfo, coeff=[1., 1.]):
    ...
    gt_CGNormal = compute_mesh_vert_normals(gt_CGPos, G_TMesh.coarse_faces_tensor)
    ...
    # loss_c_smooth = \
    #     smooth_coeff * smooth_loss(coarse_results, G_TMesh.coarse_faces_tensor, G_TMesh.coarse_average_weights)

    Loss_coarse = loss_c_geo + loss_c_normal
    ...
    #Loss_fine = Loss_fine + strain_coeff * f_pstrain_loss / patch_BatchSize
...
if_adj_lr = True
if_lr_adj_start_slow = True
lr_scale = 0.5
lr_adjust_freq = 80000
print('lr_adj_freq:', lr_adjust_freq)
saveKK = 10000

if_adj_coeff = True
adj_coeff_freq = 80000
train_coeff = [1.9, 0.1]
...
for itt in range(bit+1, total_iterations+1):
    ...
    if itt % 1000 == 0 or itt == total_iterations:
        if itt % 1000 == 0 or itt == total_iterations:
            ckp_fname = '../ckp/all/All_temp.ckp'
            if itt % saveKK == 0 or itt == total_iterations:
                ckp_fname = '../ckp/all/All_' + str(itt) + '.ckp'
    ...
    if itt % saveKK == 0 or itt == total_iterations:
        ...
        save_obj('../test/1/' + str(itt) + '_' + str(case_info[frameID]) + '_' + str(case_info[-1]) + '_t.obj',
                 coarse_results.detach().cpu().numpy(), coarse_faces)
        draw_patch_images('../test/1/gt_p_' + str(itt) + '_t.jpg', pimg_gt_array)
        draw_patch_images('../test/1/p_' + str(itt) + '_t.jpg', pimg_rst_array)
        ...
        save_obj('../test/1/' + str(itt) + '_' + str(t_case_info[frameID]) + '_' + str(t_case_info[-1]) + '_e.obj',
                 coarse_results.detach().cpu().numpy(), coarse_faces)
        draw_patch_images('../test/1/gt_p_' + str(itt) + '_e.jpg', pimg_gt_array)
        draw_patch_images('../test/1/p_' + str(itt) + '_e.jpg', pimg_rst_array)
