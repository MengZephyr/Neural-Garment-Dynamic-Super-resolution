...
def layered_prepare(device, motionName, Frame0, Frame1, coarse_layerSplit=-1):
    caseName = projRoot + garmentName + materialName + motionName
    saveRoot = caseName + finePD + '_' + coarsePD + '_B/'
    ...
    for fI in FrameIDs:
        ...
        if IFLayered and coarse_layerSplit > 0:
            ...
            # colors = np.zeros((num_C, 3))
            # colors[selfcollision_edges[:, 0], 0] = 255
            # colors[selfcollision_edges[:, 1], 1] = 255
            # savePly('./test/CC_CPos.ply', float_tensor_to_numpy(CPos), colors, CoarseFaceArray)
            # exit(1)
        else:
            selfcollision_edges = None

        '''Ground truth'''
        GPos = readObj_vert_feats(
            caseName + '/PD' + finePD + '/PD' + finePD + '_' + str(fI).zfill(7) + '.obj', device)
        CGPos = downsample_from_fine(GPos)
        ...
        with open(saveRoot + str(fI).zfill(7) + '.npy', 'wb') as f:
            ...
            '''ground truth'''
            np.save(f, float_tensor_to_numpy(CGPos))
...
motionName = '/0_House_Dancing/'
Frame0 = 1
Frame1 = 500

layered_prepare(device, motionName, Frame0, Frame1, layerSplit_vID)
